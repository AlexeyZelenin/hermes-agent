...
export const ru: Translations = {
  ...
  app: {
    ...
    pendingReload: {
      tooltip: "Движок изменился — перезагрузите, чтобы применить",
      confirmTitle: "Перезапустить движок?",
      confirmBody:
        "Движок изменился, и новый код заработает только после перезапуска. Текущие задачи не потеряются — они продолжатся в фоне и переживут перезапуск.",
      confirmButton: "Перезапустить движок",
    },
    nav: {
      analytics: "Аналитика",
      chat: "Чат",
