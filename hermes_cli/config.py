...
DEFAULT_CONFIG = {
    ...
    "auxiliary": {
        ...
        # call.
        "transient_retries": 2,
        # Last-resort fallback to the leased Claude Code subscription pool (ACP,
        # haiku) for the listed aux tasks, tried ONLY after every configured,
        # paid, and free provider fallback is exhausted. Keeps these critical
        # functions alive during a total paid-provider outage (payment errors,
        # revoked OAuth, no local model) without depending on external keys — at
        # the cost of subscription quota, which is why it is last. No-op on hosts
        # with no Claude Code subscription pockets logged in. Empty the list to
        # disable; add task names (e.g. "title_generation") to extend it.
        "subscription_last_resort": ["triage_specifier", "kanban_decomposer", "compression"],
        "vision": {
    ...
    #
    # Per-platform notification opt-out is handled by the kanban dashboard
    ...
    "kanban": {
        ...
        # Self-redeploy: how the embedded dispatcher reacts when it notices
        # (once per tick) that hermes engine code on disk changed AFTER the
        # gateway booted — e.g. a merge landed while the gateway was running,
        # so the freshly-merged dispatcher/executor code sits dead in memory.
        #   "off"          — do nothing.
        #   "notify"       — raise a loud "engine changed, restart pending"
        #                    finding (WARNING log + best-effort operator push).
        #   "safe-restart" — additionally self-restart via the gateway restart
        #                    path, but only at a quiet moment (no in-gateway
        #                    agent/cron/API work in flight); otherwise defer to
        #                    a later tick. Detached worker subprocesses survive
        #                    a restart and are never interrupted.
        # Re-read live every tick (an unknown value falls back to "notify", so a
        # typo can never auto-restart). Debounced to one finding per merge burst.
        "auto_redeploy": "notify",
