...
def test_specify_task_happy_path(kanban_home):
    with kb.connect() as conn:
        tid = kb.create_task(conn, title="rough", triage=True, created_by="test")
...
def test_specify_task_falls_back_to_body_only_on_bad_json(kanban_home):
    with kb.connect() as conn:
        tid = kb.create_task(conn, title="keep title", triage=True, created_by="test")
...
def test_specify_task_rejects_non_triage_task(kanban_home):
    with kb.connect() as conn:
        tid = kb.create_task(conn, title="ready task", created_by="test")
...
def test_list_triage_ids(kanban_home):
    with kb.connect() as conn:
        a = kb.create_task(conn, title="a", triage=True, created_by="test")
        b = kb.create_task(conn, title="b", triage=True, tenant="proj-1", created_by="test")
        kb.create_task(conn, title="c", created_by="test")  # not triage — excluded
...
def test_cli_specify_all_success_and_json(kanban_home, capsys):
    with kb.connect() as conn:
        a = kb.create_task(conn, title="a", triage=True, created_by="test")
        b = kb.create_task(conn, title="b", triage=True, created_by="test")
...
def test_cli_specify_all_returns_1_when_every_task_fails(kanban_home, capsys):
    with kb.connect() as conn:
        kb.create_task(conn, title="a", triage=True, created_by="test")
        kb.create_task(conn, title="b", triage=True, created_by="test")
...
def test_cli_specify_tenant_filter(kanban_home, capsys):
    with kb.connect() as conn:
        outside = kb.create_task(conn, title="outside", triage=True, created_by="test")
