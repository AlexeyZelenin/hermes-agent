...
def test_attach_job_urls_fills_missing_links():
    ...
    # First item gets the per-job URL
    assert items[0].link == "https://run/1/job/2"
    assert items[0].link_label == "View job"
    # Second item keeps its existing link
...
def test_attach_job_urls_fallback_to_run_url():
    ...
    assert items[0].link == "https://fallback"
    assert items[0].link_label == "View job"
