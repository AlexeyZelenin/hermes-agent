...
def render_comment(items: list[ReviewItem], pending_jobs: list[str] | None = None) -> str:
    ...
    pending_text = f"\n<sub>still waiting for {len(pending)} ci job{'s' if len(pending) != 1 else ''}" if pending else ""
...
def _attach_job_urls(items: list[ReviewItem], job_urls: dict[str, str], run_url: str) -> None:
    """Fill in per-job log links for items that don't have one yet.
    ...
    job names in ``job_urls``. Items that already have a ``link`` are left
    alone (e.g. ci-timings' report URL).
    """
    ...
    for item in items:
        ...
        if src in norm_urls:
            item.link = norm_urls[src]
            item.link_label = "View job"
            continue
        ...
        if not item.link and item.source and run_url:
            item.link = run_url
            item.link_label = "View job"
